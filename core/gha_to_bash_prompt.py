...
def prompt1(gha : str, files: str) -> Tuple[str, str, str, str, str]:
    ...
    identify = guidance(dedent('''
        ...
        * `build.Dockerfile`: A dockerfile with the correct base image to support the build steps. This includes any programming language tool and any dependencies needed for `build.sh`. If no special dependencies are needed choose alpine. 
        * `build.sh` A bash file that runs the steps of the build process. It will run inside `build.Dockerfile` in the working directory of the repository. If no build steps need to run inside the container, you just include an `echo` as a placeholder. 

        Other files will exist in the repository. Code files and other assets and possibly an application Dockerfile. The application Dockerfile you can call `app`.
        ...
        * You do not need to chmod `build.sh` or `run.sh`. That is taken care of.
        ...
        Remember `build.Dockerfile` should work without volume mounting: files that are needed need to be copied in.
